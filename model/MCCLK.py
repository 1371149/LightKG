...
class GraphConv(nn.Module):
    ...
    def build_adj(self, context, topk):
        ...
        batch_size = 256
        knn_val_list, knn_index_list = [], []
        for x in torch.split(context_norm, batch_size, dim=0):
            sim = torch.mm(x, context_norm.transpose(1, 0))
        ...
        knn_val, knn_index = torch.cat(knn_val_list, dim=0), torch.cat(knn_index_list, dim=0)
