...
class KGRec(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.ui_mat = dataset.inter_matrix(form="coo").astype(
            np.float32
        )
        self.adj_mat = self._make_si_norm_adj(self.ui_mat)
        
        
        self.inter_edge, self.inter_edge_w = self._convert_sp_mat_to_tensor(
            self.adj_mat)

        self.kg = dataset.kg_graph(
            form="coo", value_field="relation_id"
        )
        self.edge_index, self.edge_type =torch.tensor(np.array([self.kg.row,self.kg.col])).to(self.device),torch.tensor(self.kg.data).to(self.device)
        ...
        self.enc_edge_index_2 = None
        self.enc_edge_type_2 = None
        self.inter_edge_2 = None
        self.inter_edge_w_2 = None
    ...
    def calculate_loss(self, interaction):
        ...
        # rec task
        self.enc_edge_index_2 = enc_edge_index
        self.enc_edge_type_2 = enc_edge_type
        self.inter_edge_2 = inter_edge
        self.inter_edge_w_2 = inter_edge_w
        self.entity_embedding, self.user_embedding = self.gcn(user_emb,
                                                item_emb,
                                                enc_edge_index,
                                                enc_edge_type,
                                                inter_edge,
        ...
        pos_e, neg_e = self.entity_embedding[pos_item], self.entity_embedding[neg_item]

        bpr_loss = self.create_bpr_loss(u_e, pos_e, neg_e)

        # MAE task with dot-product decoder
        # mask_size, 2, channel
        node_pair_emb = self.entity_embedding[masked_edge_index.t()]
        # mask_size, channel
        ...
        self.user_embedding,self.entity_embedding = None,None

        return bpr_loss + mae_loss + cl_loss
    ...
    def predict(self, interaction):
        ...
        if self.test == False:
            ...
            with torch.no_grad():
                self.entity_embedding, self.user_embedding = self.gcn(user_emb,
                                                item_emb,
                                                self.enc_edge_index_2,
                                                self.enc_edge_type_2,
                                                self.inter_edge_2,
                                                self.inter_edge_w_2,
                                                mess_dropout=False,
                                                )
        u_e = self.user_embedding[user]
        i_e = self.entity_embedding[item]
